...
const MediaContent = ({ mediaType }: { mediaType: 'video' | 'image' }) => {
  ...
  return (
    <div className='max-w-4xl mx-auto'>
      <h2 className='text-3xl font-bold mb-6 text-black dark:text-slate-900'>
        About This Component
      </h2>
      <p className='text-lg mb-8 text-black dark:text-slate-900'>
      ...
      <div className='fixed top-4 right-4 z-50 flex gap-2'>
        <button
          ...
          className={`px-4 py-2 rounded-lg ${
            mediaType === 'video'
              ? 'bg-white text-black'
              : 'bg-black/50 text-slate-900 border border-slate-300'
          }`}
        >
          Video
        </button>
